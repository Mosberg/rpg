...
public class AdminUI {
    ...
    public static void register() {
        ...
        Registry.register(Registries.BLOCK, new Identifier("rpg", "village_manager"), VILLAGE_MANAGER_BLOCK);
        UseBlockCallback.EVENT.register((player, world, hand, hitResult) -> {
            if (!world.isClient()) {
                if (world.getBlockState(hitResult.getBlockPos()).getBlock() == VILLAGE_MANAGER_BLOCK && hand == Hand.MAIN_HAND) {
                    // Send village data to client (stub)
                    PacketByteBuf buf = new PacketByteBuf(io.netty.buffer.Unpooled.buffer());
                    buf.writeString("TODO: Village Data");
                    ServerPlayNetworking.send((ServerPlayerEntity) player, new Identifier("rpg", "village_manager_data"), buf);
